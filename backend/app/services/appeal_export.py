...
def build_order_docx(
    ...
    *,
    ...
    still work; the title block then uses just the case title.
    """
    doc = Docx()
    _set_doc_defaults(doc)

    # Optional small "DRAFT" banner above the centred title.
    banner = doc.add_paragraph()
    banner.paragraph_format.alignment = WD_ALIGN_PARAGRAPH.CENTER
    banner.paragraph_format.space_after = Pt(4)
    br = banner.add_run("DRAFT APPELLATE ORDER")
    br.bold = True
    ...
    if section:
        ...
        r = sub.add_run(f"Appeal against order u/s {section.strip()}")
        r.italic = True
        r.font.name = BODY_FONT
        r.font.size = Pt(11)
    ...
    r = note.add_run(
        "Draft generated by BharathTax for the Commissioner (Appeals) to "
